...
/**
 * Author: Tanner Morlan
 * Class gets RSS feed, and finds start and end of elements to capture item, title, description, link, and publication date.
 */
public class RSSFeedHandler extends DefaultHandler {
    ...
    //qName = qualified name (checks to see if element needed by app--that is, "item," "title," "description," etc.)
    @Override
    ...
    public void characters(char ch[], int start, int length) {
        ...
        if (isTitle) {
            if (feedTitleHasBeenRead == false) {
                ...
                //"<" means that it is beginning of RSS element
                if (s.startsWith("<")) {
            ...
            if (dateParts.length > 8) {
                ...
                if (s.startsWith("https://sdtimes.com/feed/")) {
                    year = dateParts[5];
                    month = dateParts[6];
                    day = dateParts[7];
                }
